...
* `--anonymous-inbound`: i2p/Tor **inbound** connections are not built, so a
  node can reach hidden services but cannot be one. `--proxy` and `--tx-proxy`
  are built (see B); neither has been run against a live proxy.
